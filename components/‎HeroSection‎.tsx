...
export default function HeroSection() {
  ...
  return (
    <section
      id="home"
      className="relative overflow-hidden min-h-screen bg-gradient-to-br from-white via-[#F7F0FF] to-[#E8DEFF]"
    >
      {/* Navbar */}
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/70 backdrop-blur-lg shadow-md border-b border-gray-200">
        <div className="max-w-7xl mx-auto flex justify-between items-center px-8 py-4">
          <div
            className="text-2xl font-bold text-[#7B1E7A] tracking-tight cursor-pointer"
            onClick={() => scrollToSection("home")}
          >
            <span>B</span>etty
          </div>

          <ul className="hidden md:flex gap-10 text-[1rem] font-medium">
            {sections.map((id) => (
              <li
                key={id}
      ...
      <div className="relative min-h-screen py-44">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-lg leading-relaxed max-w-lg text-black/70">
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="px-6 py-3 rounded-full text-base font-bold bg-gradient-to-r from-[#DE8CF0] to-[#C97DF0] text-white shadow-lg transition-all duration-300"
              >
                Explore My Work
              </motion.button>
      ...
      >
        ...
        <p className="text-sm mt-1 text-black/70">Scroll to explore</p>
      </motion.div>
    </section>
  );
}
